...
<!-----主內容------>
<div class="container">
	<div class="tabTitle"><?php echo ($parent["title"]); ?> > <span class="conTitle">班級列表</span></div>                           
    <div class="Main">
		<?php if($_SESSION['roleId']== 1): ?>計畫:  
		<select id="mid">
			<?php if(is_array($al)): $i = 0; $__LIST__ = $al;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$co): $mod = ($i % 2 );++$i; if($co['lastclass'] == $id): ?><option  selected><?php echo ($co["lastclass"]); ?></option>
			<?php else: ?>
					<option ><?php echo ($co["lastclass"]); ?></option><?php endif; endforeach; endif; else: echo "" ;endif; ?> 
		</select><?php endif; ?>
			<!-- 按鈕 start -->
			<div class="buttonA">
				<input type="hidden" name="operate" id="operate2" />
				<input name="jumpUri" type="hidden" id="jumpUri" value="/index.php/Admin/Relate/myclass.html" />
			...
			<div class="tableListA">
				<table>
					...
					<?php if(isset($dataList)): if(is_array($dataList)): $i = 0; $__LIST__ = $dataList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
								<td width="4%" align="center" valign="middle"><?php echo ($p*5-5+$i); ?></td>
								<td width="3%" align="center">
									...
									<input name="orders[]" class="checkOpt" style="width:20px;" type="text" maxlength="2" AutoComplete="Off" placeholder="<?php echo ($vo["orders"]); ?>" value='<?php echo ($vo["orders"]); ?>'>
								</td>
								<td><?php echo ($vo['lastclass']); ?></td>
...
<script>
	$('#mid').change(function(){
		window.location.href="<?php echo U('Relate/myclass');?>"+'?id='+$(this).val(); 
	})
	$("#auto").click(function(){
        $("#autoform").ajaxSubmit({
            //dataType:'script',
            type: 'get',
            url: "<?php echo u('Relate/autoclass');?>",
